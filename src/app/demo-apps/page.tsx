          ...
          <section key={group.id}>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {group.items.map((c) => (
                ...
                >
                  <Card hover className="group h-full overflow-hidden transition hover:border-accent-teal/40 flex flex-col p-0">
                    {c.imageUrl && (
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <h3 className="mt-2 text-base font-semibold text-foreground leading-snug">{c.name}</h3>
                      <p className="mt-1 text-xs font-medium text-foreground/80">{c.tagline}</p>
                      <p className="mt-2 line-clamp-3 text-xs leading-relaxed text-muted-foreground flex-1">
                        {c.description}
                      </p>
                      {c.learning?.outcomes?.[0] && (
                        <p className="mt-3 line-clamp-1 text-[11px] font-medium text-accent-teal/90">
                      ...
                      <p className="mt-1 text-[10px] text-muted-foreground/80">
                        Like: {c.examples.slice(0, 2).join(", ")}
                      </p>
                    </div>
                  </Card>
                </a>

...
export function makeUnsubscribeToken(email: string, tenantId: string): string {
...
export function verifyUnsubscribeToken(email: string, tenantId: string, token: string): boolean {
  return makeUnsubscribeToken(email, tenantId) === token;
}

function buildUnsubscribeUrl(email: string, tenantId: string): string {
  const token = makeUnsubscribeToken(email, tenantId);
  return `${SITE_URL}/api/marketing/email/unsubscribe?email=${encodeURIComponent(email)}&tenant=${encodeURIComponent(tenantId)}&token=${token}`;
}

function withComplianceFooter(html: string, email: string, tenantId: string): string {
  const unsubUrl = buildUnsubscribeUrl(email, tenantId);
  return `${html}
...
export async function resolveCampaignRecipients(
  campaign: Pick<Campaign, "tenantId" | "recipients" | "includeAllLeads">
): Promise<string[]> {
  const set = new Set(campaign.recipients.map((e) => e.trim().toLowerCase()).filter(Boolean));
  if (campaign.includeAllLeads) {
    const leads = await listLeads(campaign.tenantId);
    for (const lead of leads) set.add(lead.email);
  }
  return filterSuppressed(Array.from(set), campaign.tenantId);
}

export async function sendCampaignNow(
  campaign: Pick<Campaign, "tenantId" | "subject" | "html" | "recipients" | "includeAllLeads">
  ...
  for (const email of recipients) {
    const ok = await sendMailboxMessage({
      ...
      html: withComplianceFooter(campaign.html, email, campaign.tenantId),
    });
    if (ok) sentCount += 1;
    else failedCount += 1;
  }

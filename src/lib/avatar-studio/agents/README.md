...
| Voice | `voice-agent.ts` | Freemium: user Setup custom URL → catalog env URL → free Edge/Google TTS (no GPU). | `(voiceModelId, script, tier, voiceProfileId, userEmail?)` → `GenerationResult` | 2.0.0 | Avatar Studio |
| Avatar | `avatar-agent.ts` | Freemium: user Setup custom URL → catalog env URL → free Presenter (portrait + audio, playable in-app). Optional `referenceImageUrl` for long-form lip-sync hosts. | `(…, referenceImageUrl?, userEmail?, durationHint?)` → `GenerationResult` | 2.0.0 | Avatar Studio |

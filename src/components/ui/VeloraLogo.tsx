...
export function VeloraLogo({ variant = "full", className }: VeloraLogoProps) {
  return (
    <Link href="/" className={cn("flex items-center gap-3 group", className)}>
      <svg viewBox="0 0 40 40" className="h-9 w-9 shrink-0" aria-hidden="true">
        <defs>
          <linearGradient id="velora-grad-1" x1="0%" y1="100%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#0E6B66" />
            <stop offset="100%" stopColor="#14A39B" />
          </linearGradient>
          <linearGradient id="velora-grad-2" x1="100%" y1="0%" x2="0%" y2="100%">
      ...
      {variant === "full" && (
        <span className="text-lg font-semibold tracking-tight text-foreground">
          Velora <span className="text-teal">Labs</span>
        </span>
      )}
    </Link>
  );
